...
//            var_dump($this->products);exit;
    ...
    private function getDefaultProductByPosition($product){

        $configurableProduct = $this->productRepository->getById($product->getEntityId());
        $dataChild = $configurableProduct->getTypeInstance()->getUsedProducts($configurableProduct);
        $newChildren = [];
        foreach($dataChild as $key => $child){
            $productChild = $this->productRepository->getById($child->getId());
            ...
            $newChildren[$key]["price"] = $child->getPrice();
            $newChildren[$key]["product_id"] = $child->getId();
        }
        ...
        $priceChildProduct = number_format(intval($firstEleOfSortArrNewChildren["price"]),2);
        $html = '';
        $html .= '<div class="price-box price-final_price" data-role="priceBox" data-product-id="468" data-price-box="product-id-'.$firstEleOfSortArrNewChildren["product_id"].'">';
            $html .= '<span class="normal-price">';
                $html .= '<span class="price-container price-final_price tax weee">';
                    $html .= '<span id="product-price-'.$firstEleOfSortArrNewChildren["product_id"].'" data-price-amount="'.$priceChildProduct.'" data-price-type="finalPrice" class="price-wrapper">';
                        $html .= '<span class="price">€'.$priceChildProduct.'</span>';
                    $html .= '</span>';
                $html .= '</span>';
            $html .= '</span>';
        $html .= '</div>';
    ...
    private function getCollectionData(): array
    {
        ...
        foreach ($this->getLoadedProductCollection() as $product) {
            ...
            if($product->getTypeId() == "configurable"){
                $data['price'] = $this->getDefaultProductByPosition($product);
            }else{
